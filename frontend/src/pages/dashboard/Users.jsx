...
const Users = () => {
    ...
    // Dentro do componente Users
    const handleCreate = async (newUserData) => {
        try {
            const token = localStorage.getItem('token');
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:5003';

            const response = await axios.post(`${apiUrl}/api/users`, newUserData, {
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            alert('Erro: ' + (error.response?.data?.message || 'Falha ao criar usuário'));
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`${apiUrl}/api/users/${editingUser.id}`, formData, {
                headers: { Authorization: `Bearer ${token}` },
            });
